...
static int msm_voice_route(int rx_dev_id, int tx_dev_id, int set)
{
	...
	if (tx_dev_info->opened) {
		...
		if (tx_freq && (tx_dev_info->set_sample_rate != tx_freq)) {
			rc = tx_dev_info->dev_ops.set_freq(tx_dev_info, tx_freq);
			if (rc < 0) {
				pr_err("%s:device freq failed!\n", __func__);
				return rc;
			}
			tx_dev_info->set_sample_rate = rc;
			rc = 0;
		}
	}
